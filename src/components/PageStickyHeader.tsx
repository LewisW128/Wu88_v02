...
// Shared by every page's sticky header (casino/sport/promotions, etc.) --
// once the in-page tag row identified by `anchorId` scrolls fully behind the
// header, its tags get absorbed into the header itself (chips stage) instead
// of staying a separate sticky bar. Only the anchor id and the tag row
// content differ per page.
export default function PageStickyHeader({ anchorId, chips, zIndex = 20 }: { anchorId: string; chips: React.ReactNode; zIndex?: number }) {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const anchorBottom = document.getElementById(anchorId)?.getBoundingClientRect().bottom ?? Infinity;
  ...
  return (
    <div
      ref={headerRef}
      style={{ zIndex }}
      className={`sticky top-0 flex items-center px-[40px] transition-[background-color,padding,backdrop-filter] duration-200 ${
        stage === "chips"
