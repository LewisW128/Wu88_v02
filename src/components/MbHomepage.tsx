...
// Figma's "unhover" situation (node 573:3452) for Homepage+VIP is just the
// avatar and notify bell -- name/level/balance/top-up only show on hover.
// That's the resting state once the page is scrolled; at the very top it
// stays fully expanded (matching how it's always shown elsewhere), and
// hovering re-expands it early while scrolled.
const SCROLL_COLLAPSE_THRESHOLD = 20;

function useScrolledPastTop() {
  const [scrolled, setScrolled] = useState(false);
  useEffect(() => {
    const onScroll = () => setScrolled(window.scrollY > SCROLL_COLLAPSE_THRESHOLD);
    onScroll();
...
export default function MbHomepage({ style = "Homepage", state = "Unloggin" }: MbHomepageProps) {
  ...
  if (isHomepage && state === "VIP") {
    return (
      <div
        className={`group flex items-center gap-[40px] rounded-full border border-[#dadada] bg-white/60 backdrop-blur-[20px] transition-[padding] duration-300 ${
          scrolled ? "px-[30px] py-[15px] hover:px-[40px] hover:py-[20px]" : "px-[40px] py-[20px]"
        }`}
      >
        <VipAvatar />
        <div
          className={`grid transition-[grid-template-columns] duration-300 ease-out ${
            scrolled ? "grid-cols-[0fr] group-hover:grid-cols-[1fr]" : "grid-cols-[1fr]"
          }`}
        >
          <div className="flex min-w-0 items-center gap-[40px] overflow-hidden">
            <div className="flex w-[128px] shrink-0 flex-col items-start gap-[5px]">
              <div className="flex items-center gap-[10px]">
